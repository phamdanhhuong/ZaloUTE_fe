...
class SocketService {
  ...
  connect(token: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.socket = io(process.env.NEXT_PUBLIC_SOCKET_URL || 'http://localhost:8080/chat', {
        query: { token },
  ...
  onConversationsResult(callback: (conversations: SocketConversation[]) => void): () => void {
    if (!this.socket) throw new Error('Socket not connected');
    this.socket.on(SOCKET_EVENTS.GET_CONVERSATIONS_RESULT, callback);
    return () => this.socket?.off(SOCKET_EVENTS.GET_CONVERSATIONS_RESULT, callback);
  }
